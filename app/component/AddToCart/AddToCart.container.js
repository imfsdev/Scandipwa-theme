...
export const mapDispatchToProps = (dispatch) => ({
    ...
    removeFromWishlist: (options) =>
        WishlistDispatcher.removeItemFromWishlist(dispatch, options),
...
export class AddToCartContainer extends PureComponent {
    ...
    _validateAddToCart() {
        const {
            ...
            showNotification,
            product,
            product: { type_id, variants = [], items },
        } = this.props;
        ...
        switch (type_id) {
            case CONFIGURABLE:
                ...
                const { stock_status: configurableStock } = variants[
                    configurableVariantIndex
                ];
                ...
                if (configurableStock !== 'IN_STOCK') {
                    showNotification(
                        'info',
                        __(
                            'Sorry! The selected product option is out of stock!'
                        )
                    );
                    return false;
                }
                ...
                break;
            case GROUPED:
                const isAllItemsAvailable = items.every(
                ...
                if (!isAllItemsAvailable) {
                    showNotification(
                        'info',
                        __('Sorry! Child product quantities are invalid!')
                    );
                    return false;
                }
            ...
            default:
                ...
                if (stock_status !== 'IN_STOCK') {
                    showNotification(
                        'info',
                        __('Sorry! The product is out of stock!')
                    );
                    return false;
                }
        }
        ...
        if (type_id === 'grouped') {
            ...
            Promise.all(
                items.map((item) => {
                    ...
                    const quantity =
                        groupedProductQuantity[groupedProductItem.id];
    ...
    removeProductFromWishlist() {
        ...
        const wishlistItemKey = Object.keys(wishlistItems).find((key) => {
            const {
                wishlist: { sku: wSku },
            } = wishlistItems[key];
            return wSku === sku;
